...
parser.add_argument('--date_ana_start', metavar='date_ana_start', type=str,
                    help='Start date of verification (yyyymmddhhmmss). '
                         'In case of SYNOP these determine the start and end of '
                         'the plot which contains verifications for every hour. '
                         'In case of TEMP or AIREP these determine the start '
                         'and end of the averaging period which is then '
                         'displayed as one plot.')
...
parser.add_argument('--ver_int_start', metavar='ver_int_start', type=int,
                    default=0,
                    help='Time of day (h/UTC) when verification starts, inc')
parser.add_argument('--ver_int_stop', metavar='ver_int_stop', type=int,
                    default=23,
                    help='Time of day (h/UTC) when verification stops, inc')
    ...
    if os.path.exists(savefn):
        ...
        for t in timelist:
            ...
            try:
                ...
                det_bias = ov_fof['veri_data'] - ov_fof[
                    'obs']  # neg means fc colder than obs
                ...
                if args.obs == 'TEMP':
                    ...
                    for k in range(len(lev_fof)):
                        if l < len(lev_ekf):
                            if lev_fof[k] == lev_ekf[l]:
                                match[k] = True
                                l += 1
                    assert np.array_equal(lev_fof[match],
                                          lev_ekf), 'Dims do not match'
                    ...
                    lev_fof = ov_fof['level']
                    lev_ekf = ov_ekf['level']
                    assert lev_fof.shape[0] == lev_ekf.shape[
                        0], 'Dims do not match'
                    spread.extend(list(ens_spread[active]))
                ...
                print 'Either ekf or fof file does not exsist. Skip over ' \
                      'rest of loop.'
